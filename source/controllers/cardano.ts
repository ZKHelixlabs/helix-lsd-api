...
const getSignature = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (utxosToSpend.length > 0) {
      ...
      const messageHash = ethers.utils.solidityKeccak256(["address", "uint256", "string", "address"], [req.params.evmAddress, ethers.utils.parseUnits((hstADAAmount / 1e6).toString()), req.params.txid, bridgeAddr]);
